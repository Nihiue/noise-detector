...
- 持续采集真实音频输入设备。
- 启动时采集一个判断窗口作为底噪样本，按 `RMS 均值 + K 个标准差` 计算阈值并过滤低价值音频，`K` 由 `detection.threshold_stddev_multiplier` 配置。
- 对通过门控的检测窗口做分类前音量标准化，再运行 YAMNet 分类。
...
- `已记录`：分类命中保留标签后进入录制期；触发窗口和录制期窗口都会标记为已记录，录制完成后原始录音会保存并写入事件库。
